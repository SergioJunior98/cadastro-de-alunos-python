...
def cadastro_aluno():
    ...
    print(f"Inserindo o aluno:\n"
          f"Nome: {nomeAluno} | Idade: {idadeAluno}")

    cursor.execute(
        f" INSERT INTO ALUNOS ( NOME, IDADE ) VALUES ('{nomeAluno}', {idadeAluno});")
...
def consulta_banco():
    ...
    print(f"{'ID':^{espacamentoID}} | {'NOME':^{espacamentoNome}} | {'IDADE':^{espacamentoIdade}}")
    for i in resultado:
        print(
            f"{i[0]:^{espacamentoID}} | {i[1]:<{espacamentoNome}} | {i[2]:^{espacamentoIdade}}")
    print()
    inicio_programa()
...
def inicio_programa():
    ...
    cursor.execute(
        " CREATE TABLE IF NOT EXISTS ALUNOS ( ID INTEGER PRIMARY KEY AUTOINCREMENT, NOME VARCHAR, IDADE INTEGER NOT NULL); ")
